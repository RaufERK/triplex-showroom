...
type TariffValue = 'standard' | 'advanced' | 'premium'
type RoleValue = 'owner' | 'manager'
type PaymentStatus = 'В процессе' | 'Оплачен' | 'Ожидает'
type PaymentItem = {
  id: string
  title: string
  subtitle: string
...
const industries = [
  { id: 'consult', label: 'Консалтинг и услуги' },
  { id: 'it', label: 'IT и разработки' },
  { id: 'retail', label: 'Розничная торговля' },
  { id: 'logistic', label: 'Логистика и перевозки' },
  { id: 'fintech', label: 'Финансовые сервисы' },
]

const insightTiles = [
  { id: 'revenue', title: 'Выручка', value: '₽ 2,4 млн', subtitle: 'за последние 30 дней', tone: EFontType.BRAND },
  { id: 'conversion', title: 'Конверсия заявок', value: '+12%', subtitle: 'рост за неделю', tone: EFontType.SUCCESS },
  { id: 'processing', title: 'Активных платежей', value: '48', subtitle: 'ожидают подтверждения', tone: EFontType.INFO },
]
...
const Section = ({ id, title, description, children }: SectionProps) => (
  <section id={id} className='app__section'>
    ...
    <div className='app__examples'>{children}</div>
  </section>
)
...
const ExampleCard = ({ title, description, children }: ExampleCardProps) => (
  <div className='example-card'>
    ...
    <div className='example-card__body'>{children}</div>
  </div>
)

const tariffLabels: Record<TariffValue, string> = {
  standard: 'Стандарт',
  advanced: 'Бизнес',
  premium: 'Безлимит',
}
      ...
      <aside className='app__sidebar'>
        ...
        <nav className='app__nav'>
          {sectionLinks.map((link) => (
            <a key={link.id} href={`#${link.id}`} className='app__nav-link'>
